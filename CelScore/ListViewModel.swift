...
struct ListViewModel {
    ...
    func getCelebrityStructSignal(listId: String, index: Int) -> SignalProducer<CelebrityStruct, ListError> {
        return SignalProducer { observer, disposable in
            ...
            observer.send(value: CelebrityStruct(id: celeb!.id, imageURL: celeb!.isKing ? celeb!.gold3x : celeb!.picture3x, nickname:celeb!.nickName, prevScore: celeb!.prevScore, prevWeek: celeb!.prevWeek, prevMonth: celeb!.prevMonth, sex: celeb!.sex, isFollowed: celeb!.isFollowed, isKing: celeb!.isKing))
            observer.sendCompleted()
        }
    }
}
