...
struct ListViewModel {
    ...
    func getCelebrityStructSignal(listId: String, index: Int) -> SignalProducer<CelebrityStruct, ListError> {
        return SignalProducer { observer, disposable in
            ...
            observer.send(value: CelebrityStruct(id: celeb!.id, imageURL: celeb!.picture3x, nickName:celeb!.nickName, kingName: celeb!.kingName, prevScore: celeb!.prevScore, prevWeek: celeb!.prevWeek, prevMonth: celeb!.prevMonth, index: celeb!.index, sex: celeb!.sex, isFollowed: celeb!.isFollowed, isKing: celeb!.isKing))
            observer.sendCompleted()
        }
    }
}
