...
extension SignalType where Value: Sequence, Error == NoError {
	...
	public func flatten(_ strategy: FlattenStrategy) -> Signal<Value.Generator.Element, Error> {
...
extension SignalProducerType where Value: Sequence, Error == NoError {
	...
	public func flatten(_ strategy: FlattenStrategy) -> SignalProducer<Value.Generator.Element, Error> {
		return self.flatMap(strategy) { .init(values: $0) }
	}
}
